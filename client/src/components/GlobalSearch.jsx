  ...
  return (
    <div ref={wrapperRef} className="relative w-full max-w-md">
      <input
        ...
        className="w-full rounded-lg border border-slate-300 bg-slate-50 px-3 py-2 text-sm focus:border-brand-500 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/20"
      />
      {open && query.trim() && (
        <div className="absolute left-0 right-0 top-full z-50 mt-1 max-h-96 overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-lg">
          {loading ? (
            ...
            <p className="px-4 py-3 text-sm text-slate-500">No files found</p>
          ) : (
            results.map((file) => (
              <button
                key={file._id}
