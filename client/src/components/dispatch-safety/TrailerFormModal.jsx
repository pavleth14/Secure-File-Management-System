...
export default function TrailerFormModal({ trailer, open, onClose, onSave, canEdit, canLinkFolder }) {
  ...
  useEffect(() => {
    ...
    if (trailer) {
      setForm({
        ...
        dotInspectionExpiration: toDateInput(trailer.dotInspectionExpiration),
        platesExpiration: toDateInput(trailer.platesExpiration),
        notes: trailer.notes || '',
        linkedFolderId: trailer.linkedFolderId || null,
      });
    } else {
      setForm(emptyForm);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        <h2 className="mb-4 text-lg font-semibold text-slate-900 dark:text-slate-100">
          {trailer ? 'View / Edit Trailer' : 'New Trailer'}
        </h2>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Trailer#" value={form.trailerNumber} onChange={(v) => setForm({ ...form, trailerNumber: v })} readOnly={readOnly} required />
          ...
          <Field label="Size" value={form.size} onChange={(v) => setForm({ ...form, size: v })} readOnly={readOnly} />
