export const AUDIT_CATEGORIES = {
  READ: 'read',
  UPLOAD: 'upload',
  DOWNLOAD: 'download',
  EDIT: 'edit',
  DELETE: 'delete',
  MOVE: 'move',
  PERMISSIONS: 'permissions',
  USERS: 'users',
  GROUPS: 'groups',
  FOLDERS: 'folders',
  AUTH: 'auth',
  SYSTEM: 'system',
  RECRUITING: 'recruiting',
  DISPATCH: 'dispatch',
};

export const AUDIT_ACTIONS = {
  FILE_UPLOAD: 'FILE_UPLOAD',
  FILE_DOWNLOAD: 'FILE_DOWNLOAD',
  FILE_READ: 'FILE_READ',
  FILE_EDIT: 'FILE_EDIT',
  FILE_DELETE: 'FILE_DELETE',
  FILE_MOVE: 'FILE_MOVE',
  FILE_RENAME: 'FILE_RENAME',

  MY_FILE_UPLOAD: 'MY_FILE_UPLOAD',
  MY_FILE_DOWNLOAD: 'MY_FILE_DOWNLOAD',
  MY_FILE_READ: 'MY_FILE_READ',
  MY_FILE_DELETE: 'MY_FILE_DELETE',

  FOLDER_CREATE: 'FOLDER_CREATE',
  FOLDER_RENAME: 'FOLDER_RENAME',
  FOLDER_MOVE: 'FOLDER_MOVE',
  FOLDER_DELETE: 'FOLDER_DELETE',
  FOLDER_DELETE_DENIED: 'FOLDER_DELETE_DENIED',

  USER_CREATE: 'USER_CREATE',
  USER_UPDATE: 'USER_UPDATE',
  USER_DELETE: 'USER_DELETE',

  GROUP_CREATE: 'GROUP_CREATE',
  GROUP_UPDATE: 'GROUP_UPDATE',
  GROUP_DELETE: 'GROUP_DELETE',

  PERMISSION_ADD: 'PERMISSION_ADD',
  PERMISSION_REMOVE: 'PERMISSION_REMOVE',
  PERMISSION_UPDATE: 'PERMISSION_UPDATE',
  ROLE_CHANGE: 'ROLE_CHANGE',

  LOGIN: 'LOGIN',
  LOGOUT: 'LOGOUT',
  LOGIN_FAILED: 'LOGIN_FAILED',
  PASSWORD_RESET: 'PASSWORD_RESET',

  SETTINGS_UPDATE: 'SETTINGS_UPDATE',
  SYSTEM_CONFIGURATION_CHANGE: 'SYSTEM_CONFIGURATION_CHANGE',

  LEAD_CREATE: 'LEAD_CREATE',
  LEAD_IMPORT: 'LEAD_IMPORT',
  LEAD_EXPORT: 'LEAD_EXPORT',
  LEAD_UPDATE: 'LEAD_UPDATE',
  LEAD_STATUS_CHANGE: 'LEAD_STATUS_CHANGE',
  LEAD_PROCESSING_STEP_CHANGE: 'LEAD_PROCESSING_STEP_CHANGE',
  LEAD_REASSIGN: 'LEAD_REASSIGN',
  LEAD_ARCHIVE: 'LEAD_ARCHIVE',
  LEAD_RESTORE: 'LEAD_RESTORE',
  LEAD_COMMENT_ADD: 'LEAD_COMMENT_ADD',
  LEAD_COMMENT_EDIT: 'LEAD_COMMENT_EDIT',
  LEAD_SOURCE_CREATE: 'LEAD_SOURCE_CREATE',
  LEAD_SOURCE_DELETE: 'LEAD_SOURCE_DELETE',
  LEAD_STATUS_CREATE: 'LEAD_STATUS_CREATE',
  LEAD_STATUS_UPDATE: 'LEAD_STATUS_UPDATE',
  LEAD_STATUS_DELETE: 'LEAD_STATUS_DELETE',

  OLD_LEAD_IMPORT: 'OLD_LEAD_IMPORT',
  OLD_LEAD_ASSIGN: 'OLD_LEAD_ASSIGN',
  OLD_LEAD_ROUND_ROBIN_ASSIGN: 'OLD_LEAD_ROUND_ROBIN_ASSIGN',

  TRUCK_CREATE: 'TRUCK_CREATE',
  TRUCK_UPDATE: 'TRUCK_UPDATE',
  TRUCK_DELETE: 'TRUCK_DELETE',
  TRAILER_CREATE: 'TRAILER_CREATE',
  TRAILER_UPDATE: 'TRAILER_UPDATE',
  TRAILER_DELETE: 'TRAILER_DELETE',
  DRIVER_CREATE: 'DRIVER_CREATE',
  DRIVER_UPDATE: 'DRIVER_UPDATE',
  DRIVER_DELETE: 'DRIVER_DELETE',
  ASSIGNMENT_UPDATE: 'ASSIGNMENT_UPDATE',
  ENTITY_FOLDER_LINK: 'ENTITY_FOLDER_LINK',
  ENTITY_FOLDER_UNLINK: 'ENTITY_FOLDER_UNLINK',
  LOAD_CREATE: 'LOAD_CREATE',
  LOAD_UPDATE: 'LOAD_UPDATE',
  LOAD_ARCHIVE: 'LOAD_ARCHIVE',
  LOAD_MARK_ACTIVE: 'LOAD_MARK_ACTIVE',
  LOAD_MARK_OPEN: 'LOAD_MARK_OPEN',
  LOAD_MARK_DELIVERED: 'LOAD_MARK_DELIVERED',
  LOAD_COMMENT_ADD: 'LOAD_COMMENT_ADD',
  LOAD_COMMENT_EDIT: 'LOAD_COMMENT_EDIT',
};

export const TARGET_TYPES = {
  FILE: 'file',
  PERSONAL_FILE: 'personal_file',
  FOLDER: 'folder',
  USER: 'user',
  GROUP: 'group',
  PERMISSION: 'permission',
  ROLE: 'role',
  SYSTEM: 'system',
  AUTH: 'auth',
  LEAD: 'lead',
  LEAD_SOURCE: 'lead_source',
  LEAD_STATUS: 'lead_status',
  OLD_LEAD: 'old_lead',
  TRUCK: 'truck',
  TRAILER: 'trailer',
  DRIVER: 'driver',
  ASSIGNMENT: 'assignment',
  LOAD: 'load',
};
